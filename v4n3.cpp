...
void *send_msg(void *sockt)
{
	...
	//	bzero(temp, 1024);
    		bzero(bufferc, 1024);
   		    datalen = recv( sock , bufferc, 1024,0);

 
   		    	char temproot[30];
    			char req_file_name[1024];
   				strcpy(req_file_name,bufferc);

   				strcpy(req_file_name,bufferc);

   				strcpy(temproot,user_root);
   				

   				strcat(temproot,req_file_name);
    			...
    			printf("\nrecv msg node 2 %d $$$$$$ :%s \n",sock,req_file_name);
				   ...
				   {				   
				      ...
				      while((read_bytes = read(fp, buffer, 256)) > 0 )
				      {
...
void  *connect_to_other_server(void *argsfr)
{
	 		 ...
	 		 if ((sockr = socket(AF_INET, SOCK_STREAM, 0)) < 0)
			{
			 ...
			 send(sockr , reqfilepath , strlen(reqfilepath) , 0 );
...
void  receieve_file(int sockc)
{
				...
				fp1 = open("4.mp4", O_WRONLY|O_CREAT, 0644);

				ssize_t sent_bytes, rcvd_bytes;

				printf("in receive file  %s \n",dwnldfile);
...
////	fp1 = open("6.mp4", O_WRONLY|O_CREAT, 0644);
